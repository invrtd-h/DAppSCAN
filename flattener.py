...
class SolidityFlattener:
    ...
    def _process_file(self, file_path, version, is_root=False) -> str:
        if "@openzeppelin" in file_path:
            ...
            if version >= [0, 8, 0] and version <= [0, 8, 11]:
                file_path = (
                    os.path.abspath("./openzeppelin-contracts-4.5.0")
                    + file_path.split("@openzeppelin")[1]
                )
            elif version >= [0, 6, 0] and version <= [0, 7, 6]:
                file_path = (
                    os.path.abspath("./openzeppelin-contracts-3.4.0")
                    + file_path.split("@openzeppelin")[1]
                )
            elif version >= [0, 5, 0]:
                file_path = (
                    os.path.abspath("./openzeppelin-contracts-2.5.0")
                    + file_path.split("@openzeppelin")[1]
                )
        ...
        for line in lines:
            ...
            if import_match:
                ...
                flattened_import = self._process_file(
                    imported_file_path, version, is_root=False
                )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Example: uv run flattener.py ./contracts/MyToken.sol ./flattened/MyToken_flat.sol"
        )
        sys.exit(1)
